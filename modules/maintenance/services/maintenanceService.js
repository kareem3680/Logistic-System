...
// Create Maintenance
export const createMaintenanceService = asyncHandler(async (body, userId) => {
  ...
  const maintenance = await createService(Maintenance, {
    ...body,
    statusPerTruck,
    createdBy: userId,
  });
  ...
  await logger.info("Maintenance created", { maintenanceId: maintenance._id });
...
export const getAllMaintenancesService = asyncHandler(async (req) => {
  const cacheKey = `all:${JSON.stringify(req.query)}`;

  return await cacheWrapper(
    cacheKey,
    async () => {
      const result = await getAllService(Maintenance, req.query, "maintenance");
      ...
      const statsAggregation = await Maintenance.aggregate([
...

        {
          $group: {
            ...
            upcoming: {
              $sum: {
                $cond: [{ $eq: ["$statusPerTruck.status", "upcoming"] }, 1, 0],
              },
            },
            overdue: {
              $sum: {
      ...
      await logger.info("Fetched all maintenances");
...
export const getMaintenanceByIdService = asyncHandler(async (id) => {
  return await cacheWrapper(
    `id:${id}`,
    async () => {
      const maintenance = await getSpecificService(Maintenance, id);
      ...
      await logger.info("Fetched maintenance", { id });
  ...
  async (id, body, userId) => {
    ...
    if (Array.isArray(body.statusPerTruck)) {
      for (const truckUpdate of body.statusPerTruck) {
        const truckExists = await Truck.findById(truckUpdate.truck);
        if (!truckExists) {
          throw new ApiError(
            `Truck with ID ${truckUpdate.truck} not found`,
            404
          );
        }
    ...
    if (updated) {
      ...
      await delCache("maintenances:*");
      await logger.info("Maintenance updated", { id: maintenance._id });
    }
...
// Delete Maintenance
export const deleteMaintenanceService = asyncHandler(async (id) => {
  await deleteService(Maintenance, id);
  await delCache("maintenances:*");
  await logger.info("Maintenance deleted", { id });
  return;
});
